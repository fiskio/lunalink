...
TEST_CASE("bch_encode all four FID values produce valid distinct codewords") {
  ...
  for (uint8_t fid = 0; fid < 4; ++fid) {
    REQUIRE(bch_encode(fid, 42, cws[fid].data()) == BchStatus::kOk);
...
//
// Exhaustive: generate all 512 valid codewords (FID 0-3 × TOI 0-99 = 400,
// plus we sweep the full FID×TOI input space). BCH(51,8) with the prepended
...
TEST_CASE("bch_encode minimum Hamming distance >= 5 over all valid inputs") {
  ...
  for (uint8_t fid = 0; fid < 4; ++fid) {
    for (uint8_t toi = 0; toi < 100; ++toi) {
      Entry e{};
      REQUIRE(bch_encode(fid, toi, e.cw.data()) == BchStatus::kOk);
