...
TEST_CASE("frame_build_partial returns kOk for valid inputs") {
  ...
  REQUIRE(frame_build_partial(0, 0, out.data(), out.size()) == FrameStatus::kOk);
}
...
TEST_CASE("frame_build_partial SB1 region matches standalone bch_encode") {
  ...
  REQUIRE(frame_build_partial(fid, toi, frame.data(), frame.size()) == FrameStatus::kOk);

  // Standalone BCH encode.
  std::array<uint8_t, kBchCodewordLength> bch{};
  REQUIRE(bch_encode(fid, toi, bch.data(), bch.size()) == BchStatus::kOk);
...
TEST_CASE("frame_build_partial SB1 with spec test vector (FID=0, TOI=69)") {
  ...
  std::array<uint8_t, kFrameLength> frame{};
  REQUIRE(frame_build_partial(0, 69, frame.data(), frame.size()) == FrameStatus::kOk);
...
TEST_CASE("frame_build_partial SB2-SB4 region is zero-padded") {
  ...
  REQUIRE(frame_build_partial(0, 0, frame.data(), frame.size()) == FrameStatus::kOk);
...
TEST_CASE("frame_build_partial all output values are binary {0, 1}") {
  ...
  for (uint8_t fid = 0; fid < 4; ++fid) {
    std::array<uint8_t, kFrameLength> frame{};
    REQUIRE(frame_build_partial(fid, 50, frame.data(), frame.size()) == FrameStatus::kOk);
...
TEST_CASE("frame_build_partial different FID/TOI produce different frames") {
  ...
  REQUIRE(frame_build_partial(0, 1, frame_a.data(), frame_a.size()) == FrameStatus::kOk);
  REQUIRE(frame_build_partial(0, 2, frame_b.data(), frame_b.size()) == FrameStatus::kOk);
...
// Error handling
// ---------------------------------------------------------------------------
TEST_CASE("frame_build_partial rejects null output") {
  REQUIRE(frame_build_partial(0, 0, nullptr, kFrameLength) == FrameStatus::kNullOutput);
}

TEST_CASE("frame_build_partial rejects undersized buffer") {
  std::array<uint8_t, kFrameLength - 1U> small{};
  REQUIRE(frame_build_partial(0, 0, small.data(), small.size()) == FrameStatus::kOutputTooSmall);
}
...
TEST_CASE("frame_build_partial boundary FID=3 TOI=99") {
  std::array<uint8_t, kFrameLength> frame{};
  REQUIRE(frame_build_partial(3, 99, frame.data(), frame.size()) == FrameStatus::kOk);
...
TEST_CASE("frame_build_partial exact-size buffer succeeds") {
  std::array<uint8_t, kFrameLength> frame{};
  REQUIRE(frame_build_partial(0, 0, frame.data(), kFrameLength) == FrameStatus::kOk);
}

TEST_CASE("frame_build_partial oversized buffer succeeds") {
  // Extra space beyond kFrameLength should not cause issues.
  std::array<uint8_t, kFrameLength + 100U> frame{};
  frame.fill(0xAA);
  REQUIRE(frame_build_partial(1, 50, frame.data(), frame.size()) == FrameStatus::kOk);
