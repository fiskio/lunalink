...
| C5 | BCH(51,8) codec: SB1 carries 9 bits — FID (2 bits, frame ID) + TOI (7 bits, time of interval); bit 0 (FID MSB) is prepended raw, remaining 8 bits are BCH-encoded. **Encoder** — 8-stage Fibonacci LFSR using polynomial `1+X^3+X^4+X^5+X^6+X^7+X^8` (matches Figure 8 test vector; text `763` mismatch recorded in `docs/specs-ambiguities.md`); output is 52 symbols — the 51 encoded bits each XOR'd with bit 0, with bit 0 prepended. **Decoder** — correlation-based (256 hypotheses, per spec §2.4.2.1); returns decoded 8-bit word + error flag | |
